...
"""
Mixtral 8x7B Pretraining Script with YAML and CLI Configuration Overrides.

This script provides a flexible way to pretrain Mixtral 8x7B models using Megatron-Bridge with support for
...
Examples:
    ...
    Using a custom YAML config file:
        $ torchrun --nproc_per_node=8 pretrain_mixtral_8x7b.py --config-file my_custom_config.yaml

    Using CLI overrides only:
        $ torchrun --nproc_per_node=8 pretrain_mixtral_8x7b.py model.tensor_model_parallel_size=4 train.train_iters=100000

    Combining YAML and CLI overrides (CLI takes precedence):
        $ torchrun --nproc_per_node=8 pretrain_mixtral_8x7b.py --config-file conf/my_config.yaml \\
...
Configuration Precedence:
    1. Base configuration from mixtral_8x7b_pretrain_config() recipe
...
from megatron.bridge.recipes.mixtral import mixtral_8x7b_pretrain_config as pretrain_config
...
DEFAULT_CONFIG_FILENAME: str = "mixtral_8x7b_pretrain_override_example.yaml"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Pretrain Mixtral 8x7B model using Megatron-Bridge with YAML and CLI overrides",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML OmegaConf override file. Default: conf/mixtral_8x7b_pretrain_override_example.yaml",
    )
    parser.add_argument(
        "--mock-data",
        action="store_true",
        help="Use mock data for testing (overrides any data_paths in config)",
    )
...
def main() -> None:
    """
    Entry point for the Mixtral 8x7B pretraining script.

    This function orchestrates the complete configuration workflow:
    1. Loads the base configuration from mixtral_8x7b_pretrain_config() recipe
    ...
    Examples of CLI usage:
        ...
        torchrun --nproc_per_node=8 pretrain_mixtral_8x7b.py optimizer.lr=0.0002

        # Custom config file with additional overrides
        torchrun --nproc_per_node=8 pretrain_mixtral_8x7b.py --config-file my_config.yaml train.train_iters=50000

        # Multiple overrides for distributed training (MoE-specific)
    ...
    logger.info("Megatron-Bridge Mixtral 8x7B Pretraining Script with YAML & CLI Overrides")
    logger.info("------------------------------------------------------------------------")

    # Load base configuration from the recipe as a Python dataclass
    cfg: ConfigContainer = pretrain_config(mock=args.mock_data)
