...
class InternVLCommonKwargs(TypedDict, total=False):
    """Typed options accepted by InternVL recipe helper functions."""
...
def internvl3_1b_finetune_config(**user_kwargs: Unpack[InternVLCommonKwargs]) -> ConfigContainer:
    """Return a fine-tuning config for InternVL3-1B.

    See `_internvl_common` for the full list of parameters.
    """
    recommended_kwargs: InternVLCommonKwargs = {
        "hf_path": "OpenGVLab/InternVL3-1B",
        "tensor_model_parallel_size": 1,
        "pipeline_model_parallel_size": 1,
    }
    combined_kwargs: InternVLCommonKwargs = {**recommended_kwargs, **user_kwargs}
    return _internvl_common(**combined_kwargs)


def internvl3_2b_finetune_config(**user_kwargs: Unpack[InternVLCommonKwargs]) -> ConfigContainer:
    """Return a fine-tuning config for InternVL3-2B.

    See `_internvl_common` for the full list of parameters.
    """
    recommended_kwargs: InternVLCommonKwargs = {
        "hf_path": "OpenGVLab/InternVL3-2B",
        "tensor_model_parallel_size": 1,
        "pipeline_model_parallel_size": 1,
    }
    combined_kwargs: InternVLCommonKwargs = {**recommended_kwargs, **user_kwargs}
...
def internvl3_4b_finetune_config(**user_kwargs: Unpack[InternVLCommonKwargs]) -> ConfigContainer:
    """Return a fine-tuning config for InternVL3-4B.

    See `_internvl_common` for the full list of parameters.
    """
    recommended_kwargs: InternVLCommonKwargs = {
        "hf_path": "OpenGVLab/InternVL3-4B",
...
def internvl3_8b_finetune_config(**user_kwargs: Unpack[InternVLCommonKwargs]) -> ConfigContainer:
    """Return a fine-tuning config for InternVL3-8B.
...
def _internvl_common(
    hf_path: str,
    dir: Optional[str] = None,
    name: str = "internvl_finetune",
    ...
    """
    Create a fine-tuning configuration for InternVL models using a given HuggingFace path.
    ...
    if _dataset_choice == "mock":
        ...
        dataset_cfg = HFDatasetConversationProvider(
            sequence_length=seq_length,
            hf_processor_path=_processor_model,
            dataset_id=train_data_path[0] if isinstance(train_data_path, list) else train_data_path,
            num_workers=2,
            dataloader_type="single",
            data_sharding=True,
            pin_memory=True,
        ...
        raise ValueError(f"Unknown dataset_type: {_dataset_choice}")

    # Tokenizer config (won't be used for HF path calls but Megatron infra expects it)
    ...
    # RNG config for deterministic behavior
    rng_cfg = RNGConfig(
        seed=42,
    ...
    ckpt_cfg = CheckpointConfig(
        ...
        finetune=True if pretrained_checkpoint else False,
    )
